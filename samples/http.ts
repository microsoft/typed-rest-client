...
import { IRequestOptions } from '../_build/Interfaces';
...
export async function run() {
    try {
        ...
        let res: httpm.HttpClientResponse = await httpc.get('http://httpbin.org/get');
        let body: string = await res.readBody();
        cm.outputHttpBinResponse(body, res.message);

        cm.heading('get request output body');
        res = await httpc.get('http://httpbin.org/bytes/100');
        //body = await res.readBody();

        let file: NodeJS.WritableStream = fs.createWriteStream("E:\\myfile.txt");
        file.on('open', async (fd) => {
            try {
                let stream = res.message.pipe(file);
                stream.on('finish', () => {
